...
namespace dungeon::ir {
    struct ir_builder {
        ...
        value *lower_hir_expr(hir::expr_id eid) {
            ...
            } else if (auto t = std::get_if<hir::expr::call_data>(&e.data)) {
                value *result = getValue(e.ty);
            ...
            // TODO: what is the value of if/while ? how to deal with if-else ?
            else if (auto t = std::get_if<hir::expr::if_data>(&e.data)) {
                ...
                if (t->else_body) {
                    lower_hir_expr(*t->else_body);
                    add_instr(opcode::br, nullptr, {}, br_data{.branch_id = end_lab.id});
                }
                add_instr(opcode::label, nullptr, {}, end_lab);

                return nullptr;
            } else if (auto t = std::get_if<hir::expr::while_data>(&e.data)) {
                ...
                return nullptr;
            } else if (auto t = std::get_if<hir::expr::loop_data>(&e.data)) {
                ...
                add_instr(opcode::label, nullptr, {}, body_lab);
                lower_hir_expr(t->body);
                add_instr(opcode::br, nullptr, {}, body_lab);
                add_instr(opcode::label, nullptr, {}, end_lab);
                jmp_table.pop_back();
                return nullptr;
            } else if (auto t = std::get_if<hir::expr::block_data>(&e.data)) {
                for (auto &s: t->stmts) {
                    lower_hir_stmt(s);
                }
                // todo: what if we have trailing expressions ?
                return nullptr;
            }
        ...
        void lower_hir_stmt(hir::stmt_id sid) {
            ...
            if (auto t = std::get_if<hir::stmt::let_data>(&s.data)) {
                value *res = getValue(hir_fn.get_expr(t->value.idx).ty);
            ...
            // TODO: don't forget to insert return to unit functions that don't have it there in semantics :D
